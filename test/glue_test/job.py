...
df.write.partitionBy("dea_record_start_datetime").mode("overwrite").format(
    "parquet"
).save("s3://alpha-gluejobutils/silly_folder4/")
...
df_old = drd.init_record_datetimes(
    df_old, "2018-01-01 01:00:00", col_prefix="dea_record_"
)
...
json_data2 = s3.read_json_from_s3(
...
out3 = s3.get_filepaths_from_s3_folder(
    "s3://alpha-gluejobutils/testing/data/", extension=".parquet"
)
...
out4 = s3.get_filepaths_from_s3_folder(
    "s3://alpha-gluejobutils/testing/data/diamonds_parquet", extension="parquet"
)
...
### ### ### ### ### ###
s3.copy_s3_object(
    old_s3_path="s3://alpha-gluejobutils/testing/meta_data/diamonds.json",
    new_s3_path="s3://alpha-gluejobutils/testing/data_dump/copy_test/file0.json",
)
s3.copy_s3_object(
    old_s3_path="s3://alpha-gluejobutils/testing/meta_data/diamonds.json",
    new_s3_path="s3://alpha-gluejobutils/testing/data_dump/copy_test/file1.json",
)
json0 = s3.read_json_from_s3(
    "s3://alpha-gluejobutils/testing/data_dump/copy_test/file0.json"
)
json1 = s3.read_json_from_s3(
    "s3://alpha-gluejobutils/testing/data_dump/copy_test/file1.json"
)
if json0 != json1:
...
for i in [1, 2, 3]:
    s3.copy_s3_object(
        old_s3_path="s3://alpha-gluejobutils/testing/data_dump/copy_test/file0.json",
        new_s3_path="s3://alpha-gluejobutils/testing/data_dump/copy_test/file{}.json".format(
            i
        ),
    )

s3.copy_s3_folder_contents_to_new_folder(
    "s3://alpha-gluejobutils/testing/data_dump/copy_test/",
    "s3://alpha-gluejobutils/testing/data_dump/copy_test2/",
)
for i, p in enumerate(
    s3.get_filepaths_from_s3_folder(
        "s3://alpha-gluejobutils/testing/data_dump/copy_test2/"
    )
):
...
s3.copy_s3_object(
    "s3://alpha-gluejobutils/testing/data_dump/copy_test/file0.json",
    "s3://alpha-gluejobutils/testing/data_dump/copy_test/copy_file0.json",
)
...
### ### ### ### ### ### ### ### ### ### ###
metadata = s3.read_json_from_s3(
    "s3://alpha-gluejobutils/testing/meta_data/diamonds.json"
)
schema = datatypes.create_spark_schema_from_metadata(metadata)
schema2 = datatypes.create_spark_schema_from_metadata_file(
    "s3://alpha-gluejobutils/testing/meta_data/diamonds.json"
)
if schema != schema2:
    raise ValueError(
...
schema3 = datatypes.create_spark_schema_from_metadata(
    metadata, drop_columns=["carat"], non_nullable_cols=["x", "y", "z"]
)
...
meta_ignore["columns"].append(
    {"name": "datetime_col", "type": "datetime", "description": ""}
)
...
df_ignore_out = datatypes.align_df_to_meta(
    df_ignore, meta_ignore, ignore_columns=ignored_cols
)
...
if (
    datetime.datetime.strptime(
        drd.static_record_end_datetime, utils.standard_datetime_format_python
    ).strftime(utils.standard_datetime_format_python)
    != drd.static_record_end_datetime
):
...
if (
    df.take(1)[0].asDict()["dea_record_start_datetime"].strftime("%Y-%m-%d %H:%M:%S")
    != start_datetime
):
    raise ValueError("set_record_start_date FAILURE")

df = drd.set_record_start_datetime(
    df,
    "12:34:56 31/12/2017",
...
if not isinstance(
    df.take(1)[0].asDict()["dea_record_start_datetime"], datetime.datetime
):
...
if (
    df.take(1)[0].asDict()["dea_record_end_datetime"].strftime("%Y-%m-%d %H:%M:%S")
    != end_datetime
):
...
### ### ### ### ### ### ### ### ### ###
test1_ans = spark.createDataFrame(
    [
        Row(
            dea_record_start_datetime=datetime.datetime(2018, 1, 1, 1, 0),
            dea_record_end_datetime=datetime.datetime(2018, 1, 1, 1, 23, 45),
        ),
        Row(
            dea_record_start_datetime=datetime.datetime(2018, 1, 1, 1, 0),
            dea_record_end_datetime=datetime.datetime(2999, 1, 1, 0, 0),
        ),
        Row(
            dea_record_start_datetime=datetime.datetime(2018, 1, 1, 1, 23, 45),
            dea_record_end_datetime=datetime.datetime(2999, 1, 1, 0, 0),
        ),
    ]
).select("dea_record_start_datetime", "dea_record_end_datetime")

test2_ans = spark.createDataFrame(
    [
        Row(
            dea_record_start_datetime=datetime.datetime(2018, 1, 1, 1, 0),
            dea_record_end_datetime=datetime.datetime(2018, 1, 1, 1, 23, 45),
...
test3_ans = spark.createDataFrame(
    [
        Row(
            dea_record_start_datetime=datetime.datetime(2018, 1, 1, 1, 0),
            dea_record_end_datetime=datetime.datetime(2999, 1, 1, 0, 0),
...
df_new = drd.init_record_datetimes(
    df_new, "2018-01-01 01:23:45", col_prefix="dea_record_"
)

df = df_old.union(df_new)
df = drd.update_record_end_datetime(
    df, "diamond_id", "dea_record_start_datetime", col_prefix="dea_record_"
)
...
test1 = spark.sql(
    "SELECT DISTINCT dea_record_start_datetime, dea_record_end_datetime FROM df ORDER BY dea_record_start_datetime, dea_record_end_datetime"
)
test2 = spark.sql(
    "SELECT DISTINCT dea_record_start_datetime, dea_record_end_datetime FROM df WHERE diamond_id < 1000 ORDER BY dea_record_start_datetime, dea_record_end_datetime"
)
test3 = spark.sql(
    "SELECT DISTINCT dea_record_start_datetime, dea_record_end_datetime FROM df WHERE diamond_id >= 1000 ORDER BY dea_record_start_datetime, dea_record_end_datetime"
)
...
### ### ### ### ### ### ### ### ###
overwrite_test = spark.createDataFrame(
    [
        Row(col1="0", col2="bad", col3="cat", col4=0),
...
overwrite_test_ans = spark.createDataFrame(
    [
        Row(col1="0", col2="bad", col3="cat", col4=0),
...
overwrite_test = df_transforms.apply_overwrite_dict_to_df(
    overwrite_test, "col1", overwrite_dict
)
